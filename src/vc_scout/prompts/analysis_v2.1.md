...
# Required shape - read this first

Your answer is rejected outright unless it carries all of the following, **every time,
including when the evidence is thin, contradictory or absent**. A sparse dossier changes
your *statuses and scores*; it never changes the shape of your answer.

1. **Four sections, one of each kind**: exactly one `team`, one `product`, one `market` and
   one `thesis`. Not three. Not two with a note explaining why. When a dossier says nothing
   about the team, the `team` section is still required - write that nothing was
   established, and cite the recorded unknown it rests on.
2. **All seven `score_components`**, each exactly once: `pain_roi`, `wedge`, `distribution`,
   `defensibility`, `team`, `traction`, `market_timing`. A dimension the evidence cannot
   reach is `not_assessable` with a rationale saying so - it is never omitted.
3. **Exactly two or three `recommendation_changers`.** Not one, not four. These are things a
   partner could go and verify; a thin dossier makes them easier to name, not harder.
4. **Every section and every component must be anchored**: cite at least one evidence claim
   ID, or name at least one recorded unknown reference. An unanchored statement is rejected.
5. **A recorded conflict blocks `supported`.** If a component rests on a source the dossier
   records a conflict over, either keep the status at `partially_supported` or
   `contradicted`, or record the conflict in that component's `caveats`. A conflict that is
   neither reflected in the status nor named in a caveat is rejected.

Omitting a required part to signal uncertainty is the single most common way an answer
fails. Say the uncertainty in the rationale instead.
